...
const DevExSimulator = () => {
  ...
  const allQuestions = auditData.categories.flatMap(cat => 
    cat.questions.map(q => ({ ...q, categoryLabel: cat.label }))
  );

  const handleAnswer = (option) => {
    const newAnswers = [...answers, { ...option, weight: allQuestions[currentStep].revenue_leakage_weight }];
  ...
  const calculateFinalMetrics = () => {
    ...
    return {
      ...
      wastedSpend: (leakageFactor * 1200).toFixed(0),
      supportCost: (leakageFactor * 600).toFixed(0)
    };
  };
  ...
  if (showResults) {
    ...
    return (
      <div className="bg-black text-green-500 font-mono p-8 rounded-lg border border-green-900 shadow-2xl max-w-2xl mx-auto">
        ...
        <button
          onClick={() => { setAnswers([]); setCurrentStep(0); setShowResults(false); }}
